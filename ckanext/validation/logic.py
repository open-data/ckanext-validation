...
# (canada fork only): upload and allowed domain sources
...
def resource_validation_run(context, data_dict):
    ...
    if (resource.get('url_type') != 'upload' and url_parts.netloc not in allowed_domains):
        raise t.ValidationError({'url': 'Only uploaded resources and allowed domain sources can be validated.'})
...
def resource_validation_delete(context, data_dict):
    ...
    # Removed metadata fields if they exist
    if 'validation_status' in resource:
        resource.pop(u'validation_status')
    if 'validation_options' in resource:
